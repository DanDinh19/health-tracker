...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!auth) {
      const cached = await getOuraData(userId, "readiness", startDate, endDate);
    ...
    await saveOuraData(userId, "readiness", data);
    ...
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Failed to fetch readiness" },
      { status: 500 }
    );
  }
}
