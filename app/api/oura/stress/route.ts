...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!auth) {
      const cached = await getOuraData(userId, "stress", startDate, endDate);
    ...
    }>("daily_stress", { start_date: startDate, end_date: endDate });

    const data = (json.data ?? []).map((d) => ({
      day: d.day,
      stressHigh: d.stress_high ?? 0,
      recoveryHigh: d.recovery_high ?? 0,
    })).sort((a, b) => a.day.localeCompare(b.day));

    await saveOuraData(userId, "stress", data);
    return NextResponse.json({ data });
  } catch (err) {
    const cached = await getOuraData(userId, "stress", startDate, endDate);
    if (cached.length > 0) return NextResponse.json({ data: cached });
    console.error("Oura stress error:", err);
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Failed to fetch stress" },
      { status: 500 }
    );
  }
}
