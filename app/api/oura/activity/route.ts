...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!auth) {
      const cached = await getOuraData(userId, "activity", startDate, endDate);
    ...
    const data = (json.data ?? []).map((d) => ({
      ...
      steps: d.steps ?? 0,
      score: d.score ?? 0,
      activeCalories: d.active_calories ?? 0,
    })).sort((a, b) => a.day.localeCompare(b.day));

    await saveOuraData(userId, "activity", data);
    return NextResponse.json({ data });
  } catch (err) {
    const cached = await getOuraData(userId, "activity", startDate, endDate);
    if (cached.length > 0) return NextResponse.json({ data: cached });
    console.error("Oura activity error:", err);
